-- Prova P2 - 2024
-- Prova P2 - Crislaine Souza.sql
-- Solução da Prova P2
-----------------------------------------------------------------------------------

-----------------------------------------------------------------------------------
-- Disciplina: Banco de Dados 2 (CJOBDD2)
-- Nome do aluno:Crislaine Souza
...
CREATE DATABASE LOJAMARIACJ3019608;
GO


-- Habilita o contexto do banco de dados
USE LOJAMARIACJ3019608;
GO
...
SELECT NumeroPedido      AS 'Numero do Pedido',
       CodigoProduto     AS 'Código do Produto',
	   Quantidade        AS 'Quantidade de Itens'
	   FROM ITEM_DE_PEDIDO
	   WHERE QUANTIDADE = 35;
GO
...
SELECT CodigoVendedor     AS 'Codigo dos Vendedor',
       NomeVendedor       AS 'Nome dos Vendedor',
	   Salario              AS 'Salário'
FROM VENDEDORES
WHERE NomeVendedor LIKE 'A%' OR
...
SELECT p.Numero_Pedido   AS 'Numero do Pedido',
       COUNT (DISTINTIC Itens_Pedidos.Produtos)  AS 'Quantidade de Produtos Diferentes',
       SUM(ip.Quantidade)     AS 'Total Geral de Itens',
	   Total          AS 'Total Geral'
FROM PedidosPp
...
SELECT CodigoVendedor  AS 'Código dos Vendedores',
       NomeVendedor    AS 'Nome do Vendedor',
       Salario           AS 'Salário'
FROM VENDEDORES
WHERE Salario >(SELECT SUM(Salario)/COUNT (*) FROM Vendedores);
GO
...
SELECT c.CodigoCliente  AS 'Código do Cliente',
	   ...
	   i.ValorUnitario AS 'Valor Unitário',
	   i.Quantidade    AS 'Quantidade Comprada',
	   (i.ValorUnitario * i.Quantidade)  AS 'Valor Total',
	   ped.PrazoEntrega AS 'Prazo de Entrega',
	   v.NomeVendedor  AS 'Vendedor'
FROM CLIENTES c
JOIN Pedidos ped ON c.CodigoCliente = ped.CodigoCliente,
 Itens_Pedidoi ON ped.Numero_Pedido = i.Numero_Pedido,
 Produto p ON i.Codigo_Produto = i.Codigo_Produto,
 Vendedores v ON ped.Codigo_Vendedor = v.Codigo_Vendedor
ORDER BY c.Nome_Cliente, p.Nome_Produto,
         i.Quantidade, ped.Prazo_Entrega, 
		 v.Nome_Vendedor;
GO
...
DROP DATABASE LOJAMARIACJ3019608;
GO
